...
- 正文流使用 content.snapshot 同步有界文本及工具名称，不透传 thinking、工具参数或原始 SDK 事件；历史和状态用于最终校准，撤销权限后关闭正文流。
- 消息回执仅持久化幂等键、会话及命令元数据；不持久化消息正文。重启未知结果不自动重发，要求人工核对。创建会话幂等键仍仅保留在内存。
- 已实现显式引用、差异确认、冲突拒绝、路径边界检查、会话切换、停止/引导和模型选择；自动化已覆盖核心行为，真实 Obsidian UI 和编辑器撤销尚未验收。
- 尚未实现每令牌 policy/cwd 边界、排队 UI、附件、自动检索、安全本机发现及稳定 serverId。不能将 notes 会话策略等同于整个服务的权限隔离，也不宣称 P0–P3 全部完成。
